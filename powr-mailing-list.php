<?php
    /**
     * @package POWr Mailing List
     * @version 1.1
     */
    /*
    Plugin Name: POWr Mailing List
    Plugin URI: http://www.powr.io
    Description: Create and add a mailing list signup to your web page in seconds! Add the widget to your theme, or create a Mailing List on ANY page or post by using the shortcode [powr-mailing-list]. Then, simply visit your site and click the settings icon to customize your Mailing List right in the page. Many more plugins & tutorials at POWr.io.
    Author: POWr.io
    Version: 1.1
    ...
    */
    ...
    class Powr_Mailing_List extends WP_Widget{
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_mailing_list',
                             __( 'POWr Mailing List' ),
                             array( 'description' => __( 'Mailing List by POWr.io') )
        );
      }
      //This prints the div
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-mailing-list' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ?>
        <p>
          No need to edit here - just click the gears icon on your Mailing List.
        </p>
        <p>
    ...
    function register_powr_mailing_list() {
      register_widget( 'Powr_Mailing_List' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_mailing_list' );
    //Create short codes for adding plugins anywhere:
    function powr_mailing_list_shortcode( $atts ){
      if(isset($atts['label'])){
        $label = $atts['label'];
      }else{
        $label = '';
      }
      return "<div class='powr-mailing-list' label='$label'></div>";
    }
    add_shortcode( 'powr-mailing-list', 'powr_mailing_list_shortcode' );
  ?>